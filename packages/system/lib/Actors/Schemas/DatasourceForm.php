...
class DatasourceForm
{
        ...
        foreach ($field_groups as $section_handle => $section_data) {
            ...
            if (is_array($filter_data) && !empty($filter_data)) {
                foreach ($filter_data as $filter) {
                    if (isset($fields[$filter['element-name']])) {
                    ...
                    } elseif ($filter['element-name'] == 'system:id') {
                        ...
                        $type_label->appendChild(Widget::Select(
                            ...
                            [
                                ['is', false, 'Is'],
                                ['is-not', $filter['type'] == 'is-not', 'Is not']
                            ]
                        ));
            ...
            $label->appendChild(Widget::Select('fields[sort-field]', $sort_by_options, ['class' => 'filtered']));
    ...
    protected function appendCondition(Duplicator $duplicator, $condition = [])
    {
        ...
        $label->appendChild(Widget::select('fields[conditions][logic][]', [
            ['empty', ($condition['logic'] == 'empty'), __('is empty')],
            ['set', ($condition['logic'] == 'set'), __('is set')]
        ], ['class' => 'filtered']));
        $group->appendChild($label);
    ...
    public function prepare(array $data = null)
    {
        if ($data) {
            ...
            if (isset($data['sort-field'])) {
                $this->parameters()->{'sort-field'} = $data['sort-field'];
            }

            if (isset($data['sort-order'])) {
                $this->parameters()->{'sort-order'} = $data['sort-order'];
            }

            if (isset($data['limit'])) {
                $this->parameters()->{'limit'} = $data['limit'];
            }
