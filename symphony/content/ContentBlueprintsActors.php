...
use Administration;
use DataSource;
use DataSourceInterface;
use DataSourceIterator;
use DataSourceException;
use ViewIterator;
use DOMText;
use AlertStack;
use General;
...
class ContentBlueprintsActors extends AdministrationPage
{
    ...
    public function __construct()
    {
        ...
        foreach ($extensions as $extension) {
            if (!method_exists($extension, 'getActorTypes')) {
                continue;
            }
    ...
    public function __viewIndex()
    {
        ...
        if ($datasources->length() <= 0) {
            $dsTableBody = array(Widget::TableRow(
                array(
                    Widget::TableData(__('None found.'),
                    	array(
                            'class' => 'inactive',
                            'colspan' => $colspan
                        )
    ...
    protected function __viewForm()
    {
        ...
        $save = Widget::Submit(
            'action[save]',
            ($this->editing) ? __('Save Changes') : __('Create Actor'),
            array(
                'accesskey' => 's'
            )
        );
