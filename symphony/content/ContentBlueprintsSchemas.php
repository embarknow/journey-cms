...
use Lang;
use Section;
use SectionException;
use SectionIterator;
use MessageStack;

use Embark\CMS\Fields\Controller as Fields;
use Embark\CMS\Fields\FieldSchemaInterface;
use Embark\CMS\Schemas\Controller as Schemas;
...
class ContentBlueprintsSchemas extends AdministrationPage {
    ...
    public function __viewIndex()
    {
        ...
        foreach (Schemas::findAll() as $section) {
    ...
    {
        $schema = Schemas::read($this->_context[1]);
    ...
    public function __save()
    {
        ...
        if (isset($this->_context[1])) {
            $found = Schemas::read($this->_context[1]);
        ...
        if (isset($_POST['fields'])) {
            foreach ($_POST['fields'] as $index => $fieldData) {
                $field = Fields::read($fieldData['type']);
        ...
        if (!$this->errors->valid()) {
            if ($editing) {
                $saved = Schemas::update($schema, $handle);
            }

            else {
                $saved = Schemas::create($schema, $handle);
            }
        }

        // Save was a success:
        if ($saved) {
            $schema = Schemas::read($handle);
            Schemas::sync($schema);
    ...
    protected function __form(Schema $schema, $editing = false)
    {
        ...
        foreach (Fields::findAll() as $field) {
