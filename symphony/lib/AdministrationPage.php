...
class AdministrationPage extends HTMLDocument
{
    ...
    {
        if (file_exists($output_pathname) && $unlink_existing === true) {
            unlink($output_pathname);
        }

        foreach ($files as $pathname) {
            if (!file_exists($pathname) || !is_readable($pathname)) {
                throw new Exception("File '{$pathname}' could not be found, or is not readable.");
            }
    ...
    protected function __buildNavigation()
    {
        ...
        foreach ($extensions as $e) {
            if (method_exists($e, 'fetchNavigation') === false) {
                continue;
            }
