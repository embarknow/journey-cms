...
class HTMLPage extends Page
{
    ...
    public function __buildQueryString($exclude = [])
    {
        static $q;
        ...
        if (!is_array($q)) {
            ...
            foreach ($_GET as $k => $v) {
                if (is_array($v)) {
                    $q[$k] = self::__flattenQueryArray($v, $k);
                } else {
                    $q[$k] = "{$k}={$v}";
                }
            }
    ...
    private static function __flattenQueryArray(&$array, $parent)
    {
        ...
        foreach ($array as $k => $v) {
            if (is_array($v)) {
                $values[] = self::__flattenQueryArray($v, $parent."[{$k}]");
            } else {
                $values[] = "{$parent}[{$k}]={$v}";
            }
        }
    ...
    public function checkElementsInHead($path, $attr)
    {
        foreach ($this->_head as $element) {
            if (basename($element->getAttribute($attr)) == basename($path)) {
                return true;
            }
        }
    }
    ...
    public function removeFromHead($elementName, $attribute = null, $attributeValue = null)
    {
        foreach ($this->_head as $index => $element) {
            ...
            if (!is_null($attribute) && !is_null($attributeValue)) {
                ...
                if (is_null($value) || $attributeValue != $value) {
                    continue;
                }
            }
