...
use FrontendPageNotFoundExceptionHandler;
use DOMDocumentFragment;
use XSLProc;
use XSLProcException;

use Embark\CMS\Actors\Controller as Actors;
...
class View
{
    ...
    public function render(ParameterPool $Parameters, XMLDocument $Document = null, DocumentHeaders $Headers = null)
    {
        Profiler::begin('Preparing view');

        $outputParameters = new ParameterPool;
        ...
        foreach ($actors as $handle) {
            $actors_pool[$handle] = Actors::read($handle);
        ...
        if (!empty($actors_ordered)) {
            foreach ($actors_ordered as $handle) {
                ...
                } elseif ($fragment instanceof DOMDocument && !is_null($fragment->documentElement)) {
